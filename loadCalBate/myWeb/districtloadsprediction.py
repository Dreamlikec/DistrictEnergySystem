...
from myWeb.cityarch import *
from myWeb.distanceformula import *


def neighbor_search(prediction_point, metric, n, mysqldb):
    # 查询汇总表，汇总表的名称为‘xx’，可根据需要自行修改
    summary_rows = mysqldb.query( "select * from xx" )
    ...
    # prediction_point为设计人员提供的建筑信息列表（此列表中不包含城市和建筑类型），注：此列表不包含OTTV值，而应该通过计算得出 设计人员输入列表的顺序为 heding = ['城市','建筑类型','面积',
    # '建筑体形系数','夏季室内设定温度(℃)','冬季室内设定温度(℃)','室内人员密度(m2/p)','照明功率密度(W/m2)','设备功率密度(W/m2)','窗墙比','窗的太阳得热系数shgc',
    # '窗的传热系数(W/m2·℃)','墙的传热系数(W/m2·℃)']
    city_index = get_cityindex( prediction_point )
    tlw = city_index[0]
    tlf = city_index[1]
    si = city_index[2]
    # tlw为墙的平均温差，tlf为窗的平均传热温差，si为所在城市的平均日射负荷强度，这些值与所在城市有关
    ottv = (1 - prediction_point[9]) * prediction_point[12] * tlw + prediction_point[9] * prediction_point[11] * tlf + \
           prediction_point[9] * prediction_point[10] / 0.85 * si
...
def Loads_predict(prediction_point, neighbor, mysql):
    # 获得mysql中的城市及建筑类型索引
    city_arch_type = get_cityarchtype( prediction_point )
    # 得到建筑的空调面积
    ...
    # 如果距离不为0，则返回相近的n个点
        ...
        for i in range( 1, n ):
            ...
            # 将列表的第2列和第3列乘以权重和面积
            prediction[:, [0, 1]] += np.around( rows[:, [1, 2]].astype( np.float ) *
                                                weight[i] * area / totalweight, decimals=2 )
