...
if __name__ == "__main__":
    #建立一个mysql的连接，连接到阿里云的数据库中，当前我们的数据库的名称为“district_loads_forecast”
    mysqldb = MySql(host="rm-8vbbe68p76ru6v950mo.mysql.zhangbei.rds.aliyuncs.com", 
    username="energyinstitute", password="3edc$RFV", port=3306, database="district_loads_forecast")
    #['城市','建筑类型','面积','建筑体形系数','夏季室内设定温度(℃)','冬季室内设定温度(℃)','室内人员密度(m2/p)',
    # '照明功率密度(W/m2)','设备功率密度(W/m2)','窗墙比','窗的太阳得热系数shgc','窗的传热系数(W/m2·℃)','墙的传热系数(W/m2·℃)']

    #生成一个测试点
    predictlist = [['青岛', '住宅建筑', 137000, 0.3, 26, 20, 30, 8, 8, 0.4, 0.4, 2.7, 0.55],
       ['徐州', '住宅建筑', 98000, 0.3, 26, 20, 30, 8, 8, 0.4, 0.4, 2.7, 0.55],
       ['临沂', '住宅建筑', 51000, 0.3, 26, 20, 30, 8, 8, 0.4, 0.4, 2.7, 0.55],
       ['临沂', '住宅建筑', 68000, 0.3, 26, 20, 30, 8, 8, 0.4, 0.4, 2.7, 0.55]]
    data = pd.DataFrame()
    for i in range(len(predictlist)):
        #找到与其相似的4个邻近点
        neighbor = neighbor_search(predictlist[i],"eudist",4,mysqldb)
        #做出负荷预测
        prediction_list = Loads_predict(predictlist[i],neighbor,mysqldb)
        #将负荷结果下载
        prediciton_df = pd.DataFrame(prediction_list)
        
        prediciton_df.iloc[:,1:] = prediciton_df.iloc[:,1:].astype(float)
        data = pd.concat([data,prediciton_df],axis=1)
    mysqldb.close()
    output_prefix = Open_directory('选择一个路径')  
    writer = pd.ExcelWriter(output_prefix+'\loads.xlsx')
